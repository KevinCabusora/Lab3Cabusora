...
main:
	...
	clr				R12
	clr				R13
	mov.w	#0x34,	R12					; these center the block
	mov.w	#0x29,	R13
...
;	Inputs:		None
;	Outputs:	None
;	Purpose:	Creates a 8x8 block on LCD display
;	Registers:	R12	(rows (horizontal))
;				R13	(columns (vertical))
;				R15	(index)
;---------------------------
displayBox:
	call	#setAddress
...
drawBox:
	mov		#NOKIA_DATA, R12
	mov		#0xFF, r13				; This represents a solid line vertically,
	call	#writeNokiaByte			; of 8 pixels,eventually we multiply this by 8
	inc		R15
	cmp		#WIDTH,	R15				; when index stops at 8, block is formed
	jne		drawBox

	pop		R13						; must pop out of stack to finish
	pop		R12
	ret
